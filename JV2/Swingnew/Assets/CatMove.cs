...
public class CatMove : MonoBehaviour
{
	...
	protected bool stick;//bool to control when the player should stay with the seat and when not to
	...
	/*	if (sBase == null)
		{
			Debug.Log("Cat does not have a spawn point");
			sBase = GameObject.FindGameObjectWithTag("Swing").transform;
		}
		Relocate();*/
    }
    ...
    public void OnTriggerEnter(Collider other)
    {
        if(other.gameObject.tag == "Trigger")
        {
			...
			moveSpeed = 1;//so player won't place himself back at seat when trjectory has been completed

            Debug.Log("I am sitting on a Swing.");
        }
        ...
        if (other.gameObject.tag == "EndNodePoint") {
    ...
    void Update()
    {
		...
		if (stick)//When Stick is true
		{
			this.gameObject.transform.position = sBase.transform.position;// Player will stay with swing seat position until...
			if(moveSpeed == 1)//this happens. Look at the OnTriggerEnter() class to understand how this happens
			{
				stick = false;
			}
		}
